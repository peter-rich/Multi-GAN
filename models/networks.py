...
def define_GDn(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    if which_model_netG == 'resnet_6blocks':
        netGDn = Decoder_G(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=3, gpu_ids=gpu_ids)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % which_model_netG)

    if len(gpu_ids) > 0:
        netGDn.cuda(gpu_ids[0])
    init_weights(netGDn, init_type=init_type)
    return netGDn

def define_GEn(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[]):
    ...
    if which_model_netG == 'resnet_6blocks':
        netGEn = Encoder_G(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=3, gpu_ids=gpu_ids)
    ...
    init_weights(netGEn, init_type=init_type)
    return netGEn
...
## wait to change
def perceptual_feature_network(which_model_netFeat, gpu_ids=[]):
    netFeat = None
    use_gpu = len(gpu_ids) > 0
...
class Decoder_G(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=3, gpu_ids=[], padding_type='reflect'):
        ...
        self.ngf = ngf
        self.gpu_ids = gpu_ids
        #self.encoder = Encoder_G(input_nc, output_nc, ngf,norm_layer, use_dropout, n_blocks, gpu_ids, padding_type)
    ...
    def forward(self, input):
        ...
        # print('encoder_img:',encoder_img.size())
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[]):
        ...
        kw = 4
        padw = 1#expend the boundary int(np.ceil(kw-1/2))=2
        sequence = [
...
###
###Add two to 3 ResNet34
###three Encoder and Decoder
###
